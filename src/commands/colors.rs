...
use poise::serenity_prelude::{Color, CreateAttachment, CreateEmbed, CreateEmbedAuthor, CreateEmbedFooter, EditRole, Embed, Mentionable, RoleId, Timestamp, User};
...
pub async fn info(
    ...
    let (exists, role_id) = color_role_exists(conn, user.id.into(), ctx.guild_id().unwrap().into()).await?;

    match exists {
        true => {
            let guild = ctx.guild_id().unwrap();
            let roles = guild.roles(&ctx.http()).await?;
            let role_id = RoleId::new(role_id.unwrap());
        ...
        false => {
            ctx.reply("That user does not seem to have a color role managed by me").await?;
            return Ok(());
        },
    };
...
pub async fn steal(
    ctx: Context<'_>,
    #[description = "The user who's color you want to steal"]
    user: User
) -> Result<(), Error> {
    ...
    let color = match get_color_code(conn, uid.into(), guid.into()).await {
        Ok(c) => c,
        Err(err) => {
            ctx.reply("User does not seem to have a color role to steal").await?;
            return Ok(());
        }
    };
    ...
    let reply = CreateReply::default().attachment(attachment).content(format!("{} just stole {} their color!", ctx.author().mention(), user.mention()));
...
async fn insert_or_update_color(ctx: Context<'_>, color: u32) -> Result<(), Error> {
    ...
    match exists {
        true => {
            ...
            let reply = CreateReply::default().content("Color Updated!").ephemeral(true);
            ctx.send(reply).await?;
        },
        false => {
            ...
            match result {
                Ok(role) => {
                    ...
                    let reply = CreateReply::default().content("Color role created!").ephemeral(true);
                    ctx.send(reply).await?;
                },
                Err(err) => {
